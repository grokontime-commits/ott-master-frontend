...
console.log('OTT Master Frontend — Phase 3E Warehouse Inspection smoke test');
...
try {
  ...
  if (!token) {
    log('warn', 'authenticated warehouse checks skipped', 'Set SMOKE_TEST_USER_EMAIL, SMOKE_TEST_USER_PASSWORD, SUPABASE_URL, and SUPABASE_ANON_KEY to check protected endpoints.');
  } else {
    const headers = { Authorization: `Bearer ${token}` };
    const me = await request('/api/v1/auth/me', { headers });
    log('pass', '/api/v1/auth/me', `roles=${(me?.data?.roles || []).map((r) => r.role_key || r).join(',')}`);
    const stats = await request('/api/v1/warehouse/stats', { headers });
    log('pass', '/api/v1/warehouse/stats', `warehouseInspections=${stats?.data?.warehouseInspections ?? stats?.data?.warehouse_inspections ?? 'loaded'}`);
    const inspections = await request('/api/v1/warehouse/inspections?limit=5&offset=0', { headers });
    log('pass', '/api/v1/warehouse/inspections', `rows=${inspections?.data?.rows?.length ?? 0}`);
    const ready = await request('/api/v1/warehouse/ready-for-release?limit=5&offset=0', { headers });
    log('pass', '/api/v1/warehouse/ready-for-release', `rows=${ready?.data?.rows?.length ?? 0}`);
  }
} catch (error) {
  log('fail', 'Phase 3E smoke test', error.message);
}

console.log('\n=== Phase 3E Frontend Smoke Summary ===');
...
console.log('\nResult: PASS — Phase 3E frontend API foundation checks completed.');

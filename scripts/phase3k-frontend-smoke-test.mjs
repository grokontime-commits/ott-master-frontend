...
console.log('OTT Master Frontend — Phase 3K Customer Portal Integration Smoke Test');
console.log(`API base: ${API_BASE}`);
console.log(`Started: ${new Date().toISOString()}\n`);

try {
  const htmlPath = new URL('../live-server/ui/customer-portal.html', import.meta.url).pathname;
  const jsPath = new URL('../live-server/ui/customer-portal.html.js', import.meta.url).pathname;
  const apiPath = new URL('../live-server/api/ott-api-client.js', import.meta.url).pathname;
  if (existsSync(htmlPath)) ok('customer-portal.html exists'); else bad('customer-portal.html missing');
  if (existsSync(jsPath)) ok('customer-portal.html.js exists'); else bad('customer-portal.html.js missing');
  if (existsSync(apiPath)) {
    const api = await readFile(apiPath, 'utf8');
    if (api.includes('customerPortalStats') && api.includes('/api/v1/customer-portal/me/mawbs')) ok('API client includes customer portal endpoints'); else bad('API client missing customer portal endpoints');
...
console.log('\nResult: PASS — Phase 3K frontend package checks completed.');

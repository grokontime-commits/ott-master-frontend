...
function caution(label, detail='') { warn++; console.log(`WARN  ${label}${detail ? ' — ' + detail : ''}`); }
...
console.log('OTT Master Frontend — Phase 3I Equipment Return Integration Smoke Test');
console.log(`API base: ${API_BASE}`);
console.log(`Started: ${new Date().toISOString()}\n`);

try {
  const root = new URL('.', import.meta.url).pathname.replace(/\/scripts\/$/, '');
  const htmlPath = new URL('../live-server/ui/equipment-return.html', import.meta.url).pathname;
  const jsPath = new URL('../live-server/ui/equipment-return.js', import.meta.url).pathname;
  const apiPath = new URL('../live-server/api/ott-api-client.js', import.meta.url).pathname;
  if (existsSync(htmlPath)) ok('equipment-return.html exists'); else bad('equipment-return.html missing');
  if (existsSync(jsPath)) ok('equipment-return.js exists'); else bad('equipment-return.js missing');
  if (existsSync(apiPath)) {
    const api = await readFile(apiPath, 'utf8');
    if (api.includes('equipmentStats') && api.includes('/api/v1/equipment/return-jobs')) ok('API client includes equipment endpoints'); else bad('API client missing equipment endpoints');
...
console.log('\nResult: PASS — Phase 3I frontend package checks completed.');
